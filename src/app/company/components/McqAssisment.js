...
const McqAssessment = () => {
  ...
  const currentSectionRef = useRef(null); // Ref for the currently displayed section/questions
  ...
  // Calculates the number of answered questions in the current section
  const answeredQuestionsInCurrentSection = currentSection
    ? currentSection.questions.filter((_, qIndex) =>
        userAnswers.hasOwnProperty(`${activeSectionIndex}-${qIndex}`)
      ).length
    : 0;

  const totalQuestionsInCurrentSection = currentSection
  ...
  const handleAnswer = (sectionIndex, questionIndex, optionLetter) => {
    ...
    setLiveRegionMessage(
      `Question ${questionIndex + 1}, option ${optionLetter} selected.`
    );
  };
  ...
  const toggleDarkMode = () => {
    ...
    setLiveRegionMessage(
      `Dark mode ${darkMode ? "deactivated" : "activated"}.`
    );
  };

  // Effect to manage focus when switching sections or showing results
  useEffect(() => {
    if (showResults && assessmentResultsRef.current) {
      assessmentResultsRef.current.focus();
      setLiveRegionMessage("Assessment results are now displayed.");
    } else if (!showResults && currentSectionRef.current) {
    ...
    >
      ...
      >
        <div className="container mx-auto flex justify-between items-center">
          <h1 className="text-2xl font-bold" id="assessment-title">
            {mcq.assessment.title}
          </h1>
          <button
          ...
          >
            {darkMode ? (
              <span role="img" aria-label="Sun emoji for light mode">
                ☀️
              </span>
            ) : (
              <span role="img" aria-label="Moon emoji for dark mode">
                🌙
              </span>
            )}
          </button>
      ...
      <main id="main-content" className="container mx-auto pt-20 pb-16 px-4">
        {/* Navigation tabs for sections */}
        <nav aria-label="Assessment Sections" className="mb-6 overflow-x-auto">
          <div
            role="tablist"
            className="flex space-x-2 md:flex-wrap md:space-y-2"
          >
            {mcq.assessment.sections.map((section, index) => (
              <button
                ...
                tabIndex={activeSectionIndex === index ? 0 : -1}
                onClick={() => setActiveSectionIndex(index)}
                className={`px-4 py-2 rounded-lg whitespace-nowrap transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              ...
              >
                ...
                <span className="sr-only">
                  {" "}
                  {
                    mcq.assessment.sections[index].questions.filter(
                      (_, qIndex) =>
                        userAnswers.hasOwnProperty(`${index}-${qIndex}`)
                    ).length
                  }{" "}
                  of {mcq.assessment.sections[index].questions.length} questions
                  answered.
                </span>
              </button>
            ))}
        ...
        {showResults ? (
          <section
            ...
            ref={assessmentResultsRef}
            tabIndex="-1" // Make this section focusable for programmatic focus
          ...
          >
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              <div
                ...
                role="img"
                aria-label={`Score: ${score.percentage} percent`}
              >
                <div
          ...
          <section
            id={`panel-${activeSectionIndex}`}
            role="tabpanel"
            aria-labelledby={`tab-${activeSectionIndex}`}
          ...
          >
            <h2 className="text-xl font-bold mb-4">
              ...
              <span className="sr-only">
                {" "}
                - {answeredQuestionsInCurrentSection} of{" "}
                {totalQuestionsInCurrentSection} questions answered.
              </span>
            </h2>
            ...
            {currentSection.questions.map((question, questionIndex) => {
              ...
              return (
                <div
                  ...
                  aria-labelledby={`question-${activeSectionIndex}-${questionIndex}-heading`}
                >
                  <h3
                    id={`question-${activeSectionIndex}-${questionIndex}-heading`}
                  ...
                  >
                    {question.options.map((option, optionIndex) => {
                      ...
                      const optionId = `option-${activeSectionIndex}-${questionIndex}-${optionLetter}`;
                      ...
                      return (
                        <div
                          ...
                          role="radio"
                          aria-checked={userAnswer === optionLetter}
                        ...
                        >
                          ...
                          <span id={`${optionId}-label`} className="flex-grow">
                            {option.substring(3)}{" "}
                            {/* Remove "A. ", "B. ", etc. */}
                          </span>
                        </div>
        ...
        <div className="flex justify-between items-center mt-6">
          <button
            ...
            className={`px-4 py-2 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${
          ...
          >
            Previous Section
            <span className="sr-only">
              Go to the previous assessment section
            </span>
          </button>
          ...
          {activeSectionIndex === mcq.assessment.sections.length - 1 ? (
            ...
            >
              Next Section
              <span className="sr-only">Go to the next assessment section</span>
            </button>
          )}
        </div>
      </main>
      ...
      <div
        ...
        role="progressbar"
        aria-valuenow={Object.keys(userAnswers).length}
        aria-valuemin="0"
        aria-valuemax={mcq.assessment.sections.reduce(
