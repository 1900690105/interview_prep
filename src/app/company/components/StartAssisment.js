  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6 bg-white rounded-lg shadow-lg mt-10 border hover:border-blue-200">
        <h1
          className="text-2xl sm:text-3xl font-bold text-center text-blue-700 mb-6"
          tabIndex="-1" // Make heading focusable for programmatic focus
          ref={formRef}
        >
        ...
        <form
          ...
          className="space-y-5"
          aria-labelledby="job-requisition-form-heading"
        >
          <p id="job-requisition-form-heading" className="sr-only">
            Please fill out the job requisition form. Fields marked with an
          ...
          <div>
            ...
            <input
              ...
              required
              aria-required="true"
              aria-invalid={!!errors.jobRole} // Indicate invalid state
              aria-describedby={errors.jobRole ? "jobRole-error" : undefined} // Link error message
            />
            {errors.jobRole && (
              <p
                id="jobRole-error"
          ...
          <div>
            ...
            <input
              ...
              aria-describedby={
                errors.questions ? "questions-error" : "questions-hint"
              }
            />
            {errors.questions && (
              <p
            ...
            {!errors.questions && (
              <p
                id="questions-hint"
                className="mt-1 text-xs text-gray-500 sr-only"
              >
                Enter a number between 5 and 15 for the number of questions per
                skill.
              </p>
            )}
          </div>

          {/* Submit Button for Requisition Form */}
          <div className="flex justify-center pt-4">
            <Button
              ...
              aria-label={
                isSubmitting
                  ? "Submitting job requisition"
                  : "Submit job requisition"
              }
              aria-live="assertive" // To announce the submitting state
              aria-atomic="true"
            >
              {isSubmitting ? (
                <span className="flex items-center">
                  ...
                  Submitting<span className="sr-only"> job requisition</span>...
                </span>
              ) : (
                "Submit Requisition"
              )}
        ...
        {skill && (
          <section
            ...
            aria-labelledby="extracted-skills-heading"
            tabIndex="-1" // Make section focusable when it appears
            ref={skillSectionRef}
          >
            <h2 id="extracted-skills-heading" className="sr-only">
              Extracted Skills
            </h2>
            <div>
              <h3 className="font-bold text-gray-800 mb-2">Hard Skills:</h3>
              {skill?.hard_skills && skill.hard_skills.length > 0 ? (
                <ul
                  className="list-disc pl-5 space-y-1"
                  aria-label="List of hard skills"
                >
                  {skill.hard_skills.map((s, index) => (
                    <li key={index}>{s}</li>
                  ))}
                </ul>
              ) : (
                <p>No hard skills identified.</p>
              )}
            </div>
            ...
            <div>
              ...
              {skill?.soft_skills && skill.soft_skills.length > 0 ? (
                <ul
                  className="list-disc pl-5 space-y-1"
                  aria-label="List of soft skills"
                >
                  {skill.soft_skills.map((s, index) => (
                    <li key={index}>{s}</li>
                  ))}
                </ul>
              ) : (
                <p>No soft skills identified.</p>
              )}
            </div>
            ...
            <div className="mt-4 col-span-full">
              {data.mcq && data.other ? (
                ...
                <div className="flex flex-col sm:flex-row gap-2 mt-2">
                  <Button
                    onClick={generateMCQQuestion}
                    disabled={load.mcq || load.other}
                    className={
                      load.mcq
                        ? "bg-blue-400 cursor-not-allowed"
                        : "bg-blue-600 hover:bg-blue-700"
                    ...
                    aria-live="assertive"
                    aria-atomic="true"
                    ref={mcqButtonRef} // Attach ref here
                  >
                    {load.mcq ? (
                      <span className="flex items-center">
                        <svg
                        ...
                        Generating
                        <span className="sr-only"> MCQ questions</span>...
                      </span>
                    ) : (
                      "MCQ Question"
                    )}
