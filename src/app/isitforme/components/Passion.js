...
import {
  ...
  AiGeneratePassionResult,
} from "../../../../config/AllAiModels";

const Passion = ({ questions, field, test, setStarted }) => {
  ...
  const handleResult = async () => {
    setLoading(true);
    const prompt = `tell me that ${field} is my ${test} or not on the basic of this questionary response,include ${test}:yes or no.reason:conclude.next:what should i do for further check to validate.
response:${JSON.stringify(results)}`;
    try {
      const result = await AiGeneratePassionResult.sendMessage(prompt);
              ...
              <div className="space-y-6">
                ...
                <Button
                  onClick={() => {
                    ...
                    if (testLevel == 4) {
                      localStorage.setItem("testLevel", 5);
                    } else {
                      if (testLevel != null) {
                        const level = parseInt(testLevel, 10) + 1;
                        localStorage.setItem("testLevel", level);
                      } else {
                        localStorage.setItem("testLevel", 2);
                      }
                    }
                  }}
