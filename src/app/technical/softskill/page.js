...
const SoftSkillFeature = ({ icon: Icon, title, description }) => (
  <div
    className="flex items-center space-x-4 p-3 bg-blue-50 rounded-lg mb-3"
    role="region"
    aria-labelledby={title}
  >
    <Icon className="h-8 w-8 text-blue-600" aria-hidden="true" />
    <div>
      <h3 id={title} className="font-semibold text-blue-900">
        {title}
      </h3>
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg">
        <Card
          className="border-2 border-blue-100 shadow-2xl rounded-2xl overflow-hidden"
          role="region"
          aria-label="Soft Skills Assessment Overview"
        >
          <header className="bg-blue-600 text-white p-6 text-center">
            <h1 id="page-title" className="text-3xl font-bold mb-2">
