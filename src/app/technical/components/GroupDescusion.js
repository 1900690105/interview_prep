    ...
    <div className="min-h-screen bg-blue-50 p-4 md:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="text-center mb-8" role="banner">
          ...
          <p
            className="text-blue-600 text-lg max-w-3xl mx-auto"
            aria-describedby="gd-description"
          >
        ...
        >
          <CardHeader>
            <CardTitle
              id="youtube-title"
              className="flex items-center gap-2 text-blue-800 text-2xl"
            >
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <YouTube
                videoId={videoId}
                opts={opts}
                title="Group discussion guide video"
              />
            </div>
            <Button
              className="mt-2"
        ...
        <section
          className="bg-white shadow-lg rounded-lg"
          role="region"
          aria-labelledby="process-overview"
        >
          <CardHeader>
            <CardTitle
              id="process-overview"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {gd.GroupDiscussion.Overview.Process.map((step) => (
                <article
                  ...
                  aria-label={`Step ${step.Step}: ${step.Action}`}
                >
        ...
        {/* Add similar roles, labels and semantic tags for other sections: 
        - Key Attributes (use role="region", aria-labelledby, <article>)
        - Tips (label articles, clear heading hierarchy)
        - Resources (label links, add alt text if needed)
        - Websites (link name clarity, screen reader-friendly anchor text)
        - Topics (screen reader-friendly topic summaries)
    */}
      </div>
    </div>
  );
