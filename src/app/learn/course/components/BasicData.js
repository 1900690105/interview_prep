  ...
  const handleNextStep = async () => {
    ...
    try {
      ...
      const courseQuery = query(
        ...
        where("topicName", "==", formData.topicName.toLowerCase()),
        where("difficulty", "==", formData.difficulty.toLowerCase()),
        where("chapters", "==", formData.chapters.toLowerCase()),
        where("video_language", "==", formData.video_language.toLowerCase()),
        where("category", "==", formData.category.toLowerCase())
      );

      const courseSnapshot = await getDocs(courseQuery);

      if (!courseSnapshot.empty) {
        // ✅ Get courseId of the existing course
        const existingCourseId = courseSnapshot.docs[0].id;

        // 💾 Save the courseId to localStorage
        localStorage.setItem("courseId", existingCourseId);

        alert("Course with this topic already exists.");
        setLoading(false);
        handleNext();
        return;
      }
      ...
      // 💾 Save generated courseId
      localStorage.setItem("courseId", courseId);
      localStorage.setItem("content", JSON.stringify(responseJson));
      setContent(responseJson);

      // 📤 Upload new course to Firestore
