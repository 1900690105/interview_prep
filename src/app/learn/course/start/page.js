...
const CoursePage = () => {
  ...
  const [activeChapterlocal, setActiveChapterlocal] = useState(0);
  ...
  useEffect(() => {
    ...
    const fetchCourseData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          setCombinedChapterData(docSnap.data());
        } else {
  ...
  const handleTranslate = async (index) => {
    ...
    setTranslate("");
    const text =
      combinedChapterData?.combinedChapterData?.[activeChapter]?.content
        ?.content?.[index]?.text;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <ChapterIndex
            combinedChapterData={combinedChapterData}
            setActiveChapter={setActiveChapter}
            setExam={setExam}
            activeChapter={activeChapter}
            activeChapterlocal={activeChapterlocal}
            exam={exam}
          />
          ...
          {exam ? (
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Code Example */}
              {combinedChapterData?.combinedChapterData?.[activeChapter]
                ?.content?.code && (
                <Card>
                  <CardHeader>
                          ...
                          <Card className="mt-5 border-blue-600 border-2">
                            ...
                            <CardContent>
                              ...
                              <div className="text-gray-600 mt-5">
                                {expandContent?.explanation?.map((exp, ind) => (
                                  <div key={ind}>
                                    <p className="font-bold">{exp.heading}</p>
                                    <p>{exp.content}</p>
                                  </div>
                                ))}
                              </div>
