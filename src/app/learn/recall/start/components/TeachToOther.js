...
import { Camera, Mic, MicOff, Square, Video, VideoOff } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Alert, AlertDescription } from "@/components/ui/alert";
import WebCam from "@/app/components/WebCam";

const TeachToOther = ({ course }) => {
    ...
    <>
      {course?.teachToOther ? (
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-medium mb-2">
                ...
                {course?.teachToOther?.questions?.[activeQuestion]?.type}
              </h3>
              <p className="text-gray-700">
                {course?.teachToOther?.questions[activeQuestion]?.question}
              </p>
              <div className="mt-2">
                <span className="text-sm text-gray-500">Key Topics: </span>
                {course?.teachToOther?.questions[
                  activeQuestion
              ...
              <Button
                ...
                onClick={() =>
                  setActiveQuestion((prev) =>
                    Math.min(
                      course?.teachToOther?.questions?.length - 1,
                      prev + 1
                    )
                  )
                }
                disabled={
                  activeQuestion === course?.teachToOther?.questions?.length - 1
                }
              >
                Next
              </Button>
        ...
        <div className="flex flex-col items-center justify-center p-4">
          ...
          <Button onClick={() => (window.history.href = "/learn/recall")}>
            Go Back{" "}
          </Button>
        </div>
      )}
