...
function FlashCard({ active2, course }) {
  ...
  const handleNextCard = () => {
    setCurrentCardIndex(
      (prevIndex) => (prevIndex + 1) % course?.flashcards?.[active2 + 1]?.length
    );
    setIsFlipped(false);
  };
  ...
  const handlePrevCard = () => {
    setCurrentCardIndex((prevIndex) =>
      prevIndex === 0
        ? course?.flashcards?.[active2 + 1]?.length - 1
        : prevIndex - 1
    );
    setIsFlipped(false);
  };
        ...
        <>
          <div className="flex flex-col items-center justify-center p-4">
            <ReactCardFlip isFlipped={isFlipped} flipDirection="vertical">
              ...
              >
                <p className="cursor-pointer">
                  {course?.flashcards?.[active2 + 1]?.[currentCardIndex]
              ...
              >
                <p className="curser-pointer">
                  {course?.flashcards?.[active2]?.[currentCardIndex]?.back ||
            ...
            <div className="mt-4 text-center text-gray-500">
              ...
              {course?.flashcards?.[active2 + 1]?.length}
            </div>
          </div>
        </>
      ) : (
